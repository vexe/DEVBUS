...
namespace DEVBUS
{
	...
	public class BetterUndo
	{
		...
		/// </summary>
		public static BetterUndo Current { get { return current ?? new BetterUndo(); } }

		/// <summary>
		/// Returns an array of all the operations currently in the undo stack
		/// </summary>
		public IOperation[] UndoStackOps { get { return undoStack.ToArray(); } }

		/// <summary>
		/// Returns an array of all the operations currently in the redo stack
		/// </summary>
		public IOperation[] RedoStackOps { get { return redoStack.ToArray(); } }
		...
		public BetterUndo()
		{
			undoStack = new Stack<IOperation>();
			redoStack = new Stack<IOperation>();
		}

		/// <summary>
		/// Registers the specified operation (pushes it to the undo stack, and clears the redo stack)
		/// </summary>
		public void Register(IOperation op)
		{
			undoStack.Push(op);
			redoStack.Clear();
		}
		...
		/// </summary>
		public void RegisterThenPerform(IOperation op)
		{
			Register(op);
			op.Perform();
		}
		...
		private bool AssertStack(int nTimes, Stack<IOperation> stack, string stackName)
		{
			if (nTimes > stack.Count)
			{
				Debug.Log(string.Format("Can't {0}: {1}",
					stackName, stack.Count == 0 ?
							   ...
							   string.Format("Number of times to {0} is higher than the current length of the {0} stack", stackName)));
				return false;
			}
			return true;
		}
		...
		{
			RegisterThenPerform(new SetVariable<T> { GetCurrent = get, SetValue = set, ToValue = toValue, OnPerformed = onPerformed, OnUndone = onUndone });
		}
		...
		{
			RegisterThenPerform(new BasicOperation { OnPerformed = onPerformed, OnUndone = onUndone });
		}
		#endregion
	}
}